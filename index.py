...
def build_model(opts):
    ...
    layer_builders = get_layer_builders(opts.method,
    ...
    for images, labels in tfds.as_numpy(batched_dataset):
        logits_samples = np.stack(
            [model.predict(images)
             for _ in range(opts.predictions_per_example)],
...
# #   dict(split='train'),
# #   dict(split='valid'),
# #   dict(split='test')] + DATA_OPTS_ROLL + DATA_OPTS_ROTATE + DATA_OPTS_ODD
...
#     if output_dir:
#         experiment_utils.record_config(
#             model_opts, output_dir+'/model_options.json')
#     return model_opts, data_opts_list


# def get_tuned_model_options(architecture, method):
#     hps = _HPS_DICT_LENET[method]
...
#     dataset_train = data_lib.build_dataset(
#         data_opts_list[0], fake_data=fake_data)
#     dataset_eval = data_lib.build_dataset(
#         data_opts_list[1], fake_data=fake_data)
